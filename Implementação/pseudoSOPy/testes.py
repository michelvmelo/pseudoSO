...
def main():
    ...
    if len(sys.argv) > 2:
        ...
        #while True:
            ...
            if len(gerenProcessos.listaProcessos) == 0 and len(gerenProcessos.filaProcessosProntos) == 0:
                break
            ...
            if len(gerenProcessos.filaProcessosProntos) > 0 or gerenProcessos.executando:
                ...
                if len(gerenProcessos.filaProcessosProntos) > 0:
                    #print gerenProcessos.filaProcessosProntos
                    maiorPrioridade = min(gerenProcessos.filaProcessosProntos, key=lambda x:x['prioridade'])
                    if maiorPrioridade['PID'] not None:
